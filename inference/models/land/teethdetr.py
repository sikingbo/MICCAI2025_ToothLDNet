...
class TeethDETR(nn.Module):
    ...
    def forward(self, features1, features2, vs_mean):
        ...
        clustered_features0, closest_cluster_idx = cluster_faces(feat2, p.permute(0, 2, 1), offsets.permute(0, 2, 1),
                                                                 vs_mean, device=device)  # B,128,T
    ...
    def inference(self, features1, features2, vs_mean):
        device = features1.device
