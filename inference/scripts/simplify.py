...
class Process:
    ...
    def simplify_points(self, mesh, target_points):
        bpy.context.view_layer.objects.active = mesh
        mod = mesh.modifiers.new(name='Decimate', type='DECIMATE')
        bpy.context.object.modifiers['Decimate'].use_collapse_triangulate = True
        #
        npoints = len((mesh.data.polygons))
        if npoints < target_faces:
            self.subsurf(mesh)
            nfaces = len(mesh.data.polygons)
        ratio = target_faces / float(nfaces)
        mod.ratio = float('%s' % ('%.6g' % (ratio)))
...
if __name__ == '__main__':
    ...
    obj_path = 'D:/dataset/Teeth3DS/data/lower'
    target_faces = 10000
    sub_files = os.listdir(obj_path)
    for filename in sub_files:
        print(filename)
        obj_file = os.path.join(obj_path, filename, filename+'_lower.obj')
        tgt_file = os.path.join(obj_path, filename, filename+'_lower_sim.obj')
        blender = Process(obj_file, target_faces, tgt_file)
        print(obj_file, target_faces, tgt_file)
