...
class Backbone_global(nn.Module):
    def __init__(self, args):
        """
        @param args: k, int, the number of neighbors.
                     dynamic, bool, if using dynamic or not.
        ...
        """
        super(Backbone_global, self).__init__()
        self.k = args.k_gnn
        self.dynamic = args.dynamic
        ...
        for _ in range(args.n_edgeconvs_backbone):
            self.convs.append(EdgeConv([channel*2, 64, 64], args.k_gnn, args.norm))
            channel = 64
...
class SemanticBranch_global(nn.Module):
    def __init__(self, args):
        super(SemanticBranch_global, self).__init__()
        self.k = args.k_gnn

        self.smlp1 = SharedMLP1d([args.emb_dims+args.n_edgeconvs_backbone*64, 256], args.norm)
        self.conv = EdgeConv([256*2, 256], args.k_gnn, args.norm)
...
class MyDGCNN_Seg(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.backbone = Backbone_global(args)      # (batch, emb_dims+n_edgeconvs*64, num_points)
